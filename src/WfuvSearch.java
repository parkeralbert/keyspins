...
public class WfuvSearch extends SpinSearch{
	...
	public ArrayList <String> getUrls (String inputPath){
		...
		try 
		{
			...
			while ((line = reader.readLine()) != null && urls.size()<4)
			{
				if (line.contains("524e") || line.contains("522a") || line.contains("5544") || line.contains("589e")) {
					urls.add(line.trim());
					...
					if (i == 0) {
						...
						for (String artistName : artistNames) {
							if (artist.equalsIgnoreCase(artistName)) {
								...
								System.out.println("WFUV spin:" + "|" + date + "|" + spin[0] + "|" + spin[1] + "|" + spin[2] + "|" + spin[3]);;
								spinData.add(spin);
							}
						}
		...
		for (String[] spin : spinData) {
			if (spin[1].equalsIgnoreCase(currentArtist)) { 
				...
				if (isDateInRange(firstDayOfWeek, lastDayOfWeek, parsedDate)) {
					spinToAdd = "Key" + "|" + artist + "|" + album + "|" + song + "|" + "WFUV" + "|" + "New York" + "|" + show + "|" + date + "|" + "-";
